...
class MySQLExternalServerRepository(IExternalServer):
    ...
    def create(self, name, api_key, base_url, status='active'):
        query = """
            ...
            VALUES (%s, %s, %s, %s)
        """
        server_id = db.execute_query(query, (name, api_key, base_url, status), commit=True)
    ...
    def find_by_id(self, server_id):
        ...
        row = db.execute_query(query, (server_id,), fetch_one=True)
        return ExternalServer(**row) if row else None


    def find_by_name(self, name):
        query = "SELECT * FROM external_servers WHERE name = %s"
        row = db.execute_query(query, (name,), fetch_one=True)
        return ExternalServer(**row) if row else None
    ...
    def update_status(self, server_id, status, new_api_key=None):
        if new_api_key:
            query = """
                ...
                WHERE id = %s
            """
            db.execute_query(query, (status, new_api_key, server_id), commit=True)
        else:
            query = """
                UPDATE external_servers
                SET status = %s
                WHERE id = %s
            """
            db.execute_query(query, (status, server_id), commit=True)
    ...
    def update_last_accessed(self, server_id):
        query = """
            ...
            WHERE id = %s
        """
        db.execute_query(query, (server_id,), commit=True)
